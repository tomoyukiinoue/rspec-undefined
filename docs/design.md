...
- Ruby: `>= 2.0.0`（gemspec の `required_ruby_version` に合わせた最低要件）。ただし実運用想定は `2.2.6+ + Rails 4.0.x`
...
- **JSON / YAML / CSV / Markdown リポート** のファイル出力（設定時のみ）。`report_format:` に `:json` / `:yaml` / `:csv` / `:markdown` を指定できる
...
lib/rspec/undefined/reporters/csv.rb      … CSV 書き出し
lib/rspec/undefined/reporters/markdown.rb … Markdown 書き出し
```

### 4.2 責務

- **Configuration**: 設定値の保持。`ENV` 参照も担う。アクセスは Mutex 保護（スレッドセーフ）
...
| `be_undefined(:cat, expected: V)` | V（Matcher なら `V.description`、生値はそのまま） | 実値 | Matcher なら `V.matches?(actual)`、生値なら `actual == V` |

カテゴリは `Symbol` のみ受理。`String` 等は `ArgumentError`。内側マッチャ（第 1 引数が Matcher）と `expected:` は排他（同時指定は `ArgumentError`）。
...
expect(users.map(&:id)).to be_undefined(match_array([1,2,3]), :order) # 内側マッチャ + カテゴリ
expect(x).to be_undefined(eq(3), :rounding)                          # 内側マッチャ
expect(x).to be_undefined(:order, expected: [1,2,3])                  # expected: 生値（== 評価）
expect(x).to be_undefined(:rounding, expected: 100)                   # expected: 生値
expect(x).to be_undefined(:order, expected: match_array([1,2,3]))     # expected: Matcher（matches? 評価）
```
...
- **Reporter 書き出し失敗**: stderr に警告を出し、スイートの結果は壊さない。書き込みは `.tmp.<pid>` への書き出し後 `File.rename` でアトミック化。失敗時は tmp ファイルを削除
...
- `gemspec`:
  - `required_ruby_version = ">= 2.0.0"`（実運用想定は `2.2.6+ + Rails 4.0.x`）
